...
<div class="min-h-screen flex flex-col justify-center items-center p-6" x-data>
    ...
    <div class="w-full max-w-md text-center">
        <div class="inline-flex items-center justify-center w-24 h-24 rounded-3xl bg-amber-100 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 mb-6 shadow-lg shadow-amber-500/10">
            <i class="ti ti-login text-5xl"></i>
        </div>
        <p class="text-7xl sm:text-8xl font-extrabold text-amber-600 dark:text-amber-400 tracking-tight">401</p>
        <h1 class="mt-4 text-2xl font-bold text-gray-900 dark:text-white">Belum Login</h1>
        <p class="mt-2 text-gray-500 dark:text-gray-400">
            {{ $exception->getMessage() ?: 'Anda harus login untuk mengakses halaman ini.' }}
        </p>
        <div class="mt-8 flex flex-col sm:flex-row gap-3 justify-center">
            <a href="{{ route('login') }}" class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-semibold text-white bg-amber-600 hover:bg-amber-700 shadow-lg shadow-amber-500/25 transition-all">
