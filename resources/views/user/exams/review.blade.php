...
@section('title', 'Review Hasil Ujian - ' . $exam->title)
    ...
    <div class="space-y-8 pb-20 p-4 md:p-8 max-w-screen-2xl mx-auto">
        ...
        <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <a href="{{ route('user.my-packages.show', $package) }}"
                class="hover:text-indigo-600 transition-colors">{{ $package->title }}</a>
            <i class="ti ti-chevron-right text-xs"></i>
        ...
        <x-card>
            <div class="space-y-4">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div>
                        <h1
                            ...
                            <span
                                class="inline-flex items-center justify-center w-9 h-9 rounded-2xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-300">
                                <i class="ti ti-key text-lg"></i>
                            </span>
                            <span>Review Hasil Ujian</span>
                        </h1>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Kunci jawaban, penjelasan, dan solusi untuk
                            semua soal</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div class="text-center">
                        <p
                            class="text-2xl font-bold {{ $trans->status === 'lulus' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}">
                            {{ ucfirst($trans->status) }}
                        </p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Status</p>
                    </div>
                    <div class="text-center">
                        <p class="text-2xl font-bold text-violet-600 dark:text-violet-400">{{ $exam->passing_grade ?? 0 }}%
                        </p>
        ...
        <div class="space-y-6">
            @forelse($detailResults as $detail)
                @php
                    ...
                    $isCorrect =
                        strtoupper(trim($detail->user_answer ?? '')) ===
                        strtoupper(trim($detail->correct_answer ?? ''));
                @endphp
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300">
                                ...
                                @if ($isCorrect)
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                                        <i class="ti ti-check mr-1"></i> Benar
                                    </span>
                                @else
                                    <span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">
                                        <i class="ti ti-x mr-1"></i> Salah
                                    </span>
                                @endif
                            </div>
                            <div
                                class="text-sm font-semibold {{ $isCorrect ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}">
                        ...
                        {{-- Question Content --}}
                        <div
                            class="p-5 md:p-6 bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800">
                            <h3
                                class="text-xs font-semibold tracking-[0.16em] uppercase text-gray-400 dark:text-gray-500 mb-3">
                                Pertanyaan
                            </h3>

                            @if ($question->question_type === 'Text')
                                <div class="prose prose-sm dark:prose-invert max-w-none">
                                    <p class="text-base font-medium text-gray-900 dark:text-white">{!! $question->question !!}
                                    </p>
                                </div>
                        ...
                        <div class="space-y-3">
                            <h4
                                class="text-xs font-semibold tracking-[0.16em] uppercase text-gray-400 dark:text-gray-500 mb-2">
                                Pilihan Jawaban
                            </h4>
                            @php
                                ...
                                $availableOptions =
                                    $lastIndex < 0 ? ['A', 'B', 'C', 'D'] : array_slice($allLetters, 0, $lastIndex + 1);
                            @endphp
                            @foreach ($availableOptions as $option)
                                ...
                                @if ($isCorrectAnswer) border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 shadow-[0_0_0_1px_rgba(16,185,129,0.4)]
                                @elseif($isUserAnswer && !$isCorrectAnswer)
                                    border-rose-500 bg-rose-50 dark:bg-rose-500/10 shadow-[0_0_0_1px_rgba(239,68,68,0.4)]
                                @else
                                    border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 @endif">
                                    <div
                                    ...
                                    @else
                                        bg-gray-50 text-gray-700 dark:bg-gray-800 dark:text-gray-200 border-gray-300 dark:border-gray-600 @endif">
                                        <span>{{ $option }}</span>
                                    </div>
                                    <div class="flex-1">
                                        <span class="text-sm text-gray-900 dark:text-gray-100">{{ $optionText }}</span>
                                    </div>
                                    @if ($isCorrectAnswer)
                                        <div
                                    ...
                                    @if ($isUserAnswer && !$isCorrectAnswer)
                                        <div
                                            class="flex items-center gap-1 text-rose-600 dark:text-rose-400 text-xs font-semibold">
                                            <i class="ti ti-x"></i>
                        ...
                        <div class="mt-6 space-y-4 pt-4 border-t border-dashed border-gray-200 dark:border-gray-800">
                            <div
                                class="flex items-center gap-2 text-xs font-semibold tracking-[0.16em] uppercase text-emerald-600 dark:text-emerald-400">
                            ...
                            <div class="grid gap-4 md:grid-cols-3">
                                <div class="md:col-span-1">
                                    <div
                                        class="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200/70 dark:border-emerald-500/40">
                                        <p
                                            class="text-[11px] font-semibold uppercase tracking-[0.14em] text-emerald-700 dark:text-emerald-300 mb-1">
                                            Kunci Jawaban
                                        </p>
                                ...
                                <div class="space-y-2 md:col-span-2">
                                    @if ($question->explanation)
                                        <div
                                            class="p-4 rounded-2xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800">
                                            <p
                                                class="text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-500 dark:text-gray-400 mb-1">
                                                Penjelasan
                                            </p>
                                    ...
                                    @if ($question->solution)
                                        <div
                                            class="p-4 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/40">
                                            <p
                                                class="text-[11px] font-semibold uppercase tracking-[0.14em] text-indigo-600 dark:text-indigo-300 mb-1">
                                                Solusi
                                            </p>
            ...
            @if ($detailResults->hasPages())
                <div class="flex items-center justify-between pt-6 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <div class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
                        <span>Soal</span>
                        <span
                            class="px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-100 font-semibold">
