    ...
    <div class="space-y-8 pb-20 p-4 md:p-8 max-w-screen-2xl mx-auto" x-data="examData()" x-init="init()">
            ...
            <div class="space-y-4">
                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div class="flex-1">
                        <h1
                            ...
                            <span
                                class="inline-flex items-center justify-center w-9 h-9 rounded-2xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 dark:bg-indigo-500/15">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    @if ($exam->passing_grade)
                        <div class="text-center">
                            <p class="text-2xl font-bold text-violet-600 dark:text-violet-400">{{ $exam->passing_grade }}%
                            </p>
        ...
        <x-card>
            ...
            <div x-show="!loading && currentQuestion" x-cloak>
                <div class="space-y-6">
                    ...
                    {{-- Navigation Buttons --}}
                    <div
                        class="flex flex-col md:flex-row md:items-center gap-3 justify-between pt-6 border-t border-gray-100 dark:border-gray-800">
                        <x-button variant="secondary" @click="previousQuestion()" x-bind:disabled="currentPage === 1"
                            x-bind:class="currentPage === 1 ? 'opacity-40 cursor-not-allowed' : ''" class="rounded-xl">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <x-button variant="primary" color="emerald" @click="submitExam(false)"
        ...
        <div x-show="submitting" x-cloak
            ...
            <div
                ...
                <div class="w-12 h-12 rounded-full border-4 border-indigo-500 border-t-transparent animate-spin">
                </div>
        ...
        function examData() {
            return {
                ...
                async loadQuestion(page) {
                    ...
                    try {
                        const response = await fetch(
                            `{{ route('user.exams.questions', ['package' => $package->id, 'exam' => $exam->id]) }}?page=${page}`, {
                                headers: {
                ...
                async submitExam(auto = false) {
                    ...
                    try {
                        const response = await fetch(
                            `{{ route('user.exams.submit', ['package' => $package->id, 'exam' => $exam->id]) }}`, {
                                method: 'POST',
                                headers: {
