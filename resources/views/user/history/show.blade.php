...
@section('content')
    <div class="space-y-8 pb-20">
        ...
        <nav class="flex items-center gap-2 text-xs font-medium text-gray-400">
            ...
            <i class="ti ti-chevron-right"></i>
            <span class="text-gray-500 dark:text-gray-500">Hasil Ujian</span>
        </nav>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                <div
                    class="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-indigo-500/25">
            ...
            <div class="flex items-center gap-3">
                ...
                <x-button variant="secondary" href="{{ route('user.history-exams.index') }}" class="rounded-xl">
        ...
        <form action="{{ route('monitor-participants.show', $userJoin) }}" method="GET"
            ...
            <div>
                <label
                    class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Periode</label>
                <select name="period"
                    class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm py-2.5 px-4 outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                <label
                    class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Tipe
                    Ujian</label>
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                        @forelse ($subjectStats as $row)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                                <td class="py-4 px-8">
                                    <span
                                        class="font-medium text-gray-900 dark:text-white">{{ $row['subject']->name }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-16 text-center">
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Tidak ada mata pelajaran di paket
                                        ini.</p>
                                </td>
                            </tr>
                        @endforelse
        ...
        <x-card :padding="false" title="Riwayat Nilai Tryout">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-50 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
                            <th class="py-4 px-8 text-[11px] font-bold uppercase text-gray-400 tracking-wider">Tryout /
                                Ujian</th>
                            <th class="py-4 px-8 text-[11px] font-bold uppercase text-gray-400 tracking-wider text-center">
                                Nilai</th>
                            <th class="py-4 px-8 text-[11px] font-bold uppercase text-gray-400 tracking-wider text-center">
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                        @forelse ($tryoutHistory as $t)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                                <td class="py-4 px-8 font-medium text-gray-900 dark:text-white">
                                    {{ $t->exam?->title ?? 'Tryout' }}</td>
                                <td class="py-4 px-8 text-center">
                                    <span
                                        class="inline-flex items-center justify-center min-w-[3rem] px-3 py-1.5 rounded-xl text-sm font-bold
                                ...
                                <td class="py-4 px-8 text-center text-sm text-gray-600 dark:text-gray-400">
                                    {{ $t->questions_answered ?? 0 }}/{{ $t->total_questions ?? 0 }}</td>
                                <td class="py-4 px-8 text-center">
                                    @php $status = $t->status ?? ''; @endphp
                                    <span
                                        class="inline-flex px-2 py-1 rounded-lg text-xs font-semibold {{ $status === 'lulus' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300' }}">
                                        {{ $status === 'lulus' ? 'Lulus' : 'Tidak Lulus' }}
                                    </span>
                                </td>
                                <td class="py-4 px-8 text-right text-sm text-gray-500 dark:text-gray-400">
                                    {{ $t->created_at?->format('d M Y, H:i') ?? '—' }}</td>
                                <td class="py-4 px-8 text-right">
                                    <x-button variant="primary" size="sm"
                                        href="{{ route('user.history.tryout-detail', [$userJoin, $t]) }}"
                                        class="rounded-lg">
                    ...
                    <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                        @forelse ($quizHistory as $q)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors">
                                ...
                                <td class="py-4 px-8 text-right text-sm text-gray-500 dark:text-gray-400">
                                    {{ $q->created_at?->format('d M Y, H:i') ?? '—' }}</td>
                            </tr>
                        @empty
                            <tr>
