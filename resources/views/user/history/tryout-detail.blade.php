...
@section('content')
    ...
    <div class="space-y-8 pb-20">
        <nav class="flex items-center gap-2 text-xs font-medium text-gray-400">
            <a href="{{ route('user.history-exams.index') }}" class="hover:text-indigo-600 transition-colors">Paket
                Diikuti</a>
            <i class="ti ti-chevron-right"></i>
            <span class="hover:text-indigo-600 transition-colors">{{ $package->title }}</span>
            <i class="ti ti-chevron-right"></i>
            <span class="hover:text-indigo-600 transition-colors">{{ $user->name }}</span>
            <i class="ti ti-chevron-right"></i>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">{{ $exam->title ?? 'Tryout' }}
                </h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $user->name }} ·
                    {{ $trans->created_at->format('d M Y, H:i') }}</p>
            </div>
            <x-button variant="secondary" href="{{ route('user.history-exams.show', $userJoin) }}" class="rounded-xl">
                <i class="ti ti-arrow-left mr-2"></i> Kembali ke Riwayat Ujian
            </x-button>
        </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="text-center">
                    <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                        {{ number_format((float) $trans->total_score, 1, ',', '') }}</p>
                ...
                <div class="text-center">
                    <p class="text-2xl font-bold text-violet-600 dark:text-violet-400">
                        {{ $trans->questions_answered ?? 0 }}/{{ $trans->total_questions ?? 0 }}</p>
        ...
        <div class="space-y-6">
            ...
            @forelse($detailResults as $detail)
                @php
                    ...
                    $isCorrect =
                        strtoupper(trim((string) ($detail->user_answer ?? ''))) ===
                        strtoupper(trim((string) ($detail->correct_answer ?? '')));
                @endphp
                <x-card>
                    <div class="space-y-4">
                        ...
                        <div
                            ...
                            @else
                                <img src="{{ $question->question_image_url ?? (str_starts_with($question->question ?? '', 'http') ? $question->question : asset('storage/' . ltrim($question->question ?? '', '/'))) }}" alt="Soal"
                                    class="max-w-full h-auto rounded-xl border border-gray-200 dark:border-gray-700"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div
                                class="p-4 rounded-xl border-2 {{ !$isCorrect ? 'border-rose-200 dark:border-rose-500/40 bg-rose-50 dark:bg-rose-500/10' : 'border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900' }}">
                                <p
                                    class="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">
                                    Jawaban Peserta</p>
                                <p class="font-bold text-lg">{{ strtoupper(trim((string) ($detail->user_answer ?? '-'))) }}
                                </p>
                            </div>
                            <div
                                class="p-4 rounded-xl border-2 border-emerald-200 dark:border-emerald-500/40 bg-emerald-50 dark:bg-emerald-500/10">
                                <p
                                    class="text-xs font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 mb-1">
                                    Jawaban Benar</p>
                                <p class="font-bold text-lg text-emerald-700 dark:text-emerald-300">
                                    {{ strtoupper(trim((string) ($detail->correct_answer ?? '-'))) }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @foreach ($availableOptions as $option)
                                @php
                                    ...
                                    $isCorrectAnswer =
                                        strtoupper(trim((string) ($detail->correct_answer ?? ''))) === $option;
                                @endphp
                                <div
                                ...
                                @else border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 @endif">
                                    ...
                                    <span
                                        class="text-sm text-gray-900 dark:text-white">{{ Str::limit($optionText, 60) }}</span>
                                    @if ($isCorrectAnswer)
                                        <span
                                            class="text-xs font-semibold text-emerald-600 dark:text-emerald-400 ml-auto">Kunci</span>
                                    @endif
                                    @if ($isUserAnswer && !$isCorrectAnswer)
                                        <span
                                            class="text-xs font-semibold text-rose-600 dark:text-rose-400 ml-auto">Peserta</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
