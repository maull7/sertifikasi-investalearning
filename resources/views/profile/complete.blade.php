        ...
        <x-card class="shadow-sm border-gray-200 dark:border-gray-800">
            ...
            <form method="post" action="{{ route('profile.complete.update') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-input label="Nomor Telepon" name="phone" placeholder="081234567890" :value="old('phone', $user->phone)" required />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-input label="Profesi" name="profesi" placeholder="Guru, Siswa, Profesional, dll" :value="old('profesi', $user->profesi)" />
                ...
                <div>
                    <x-input label="Institusi" name="institusi" placeholder="Nama sekolah / kampus / instansi"
                        :value="old('institusi', $user->institusi)" />
                </div>

                <div>
                    <x-textarea label="Alamat" name="alamat" rows="3" placeholder="Alamat lengkap"
                        required>{{ old('alamat', $user->alamat) }}</x-textarea>
                </div>
