    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            ...
            <nav class="flex items-center gap-2 text-xs font-medium text-gray-400">
                ...
                <a href="{{ route('mapping-questions.manage', $mappable) }}" class="hover:text-indigo-600 transition-colors">Mapping Soal</a>
                <i class="ti ti-chevron-right"></i>
                <span class="text-gray-500 dark:text-gray-500">Detail</span>
            </nav>
        </div>
        <div class="flex items-center gap-3">
            <form action="{{ route('mapping-questions.destroy', [$mappable, $mapping]) }}" method="POST" onsubmit="return confirm('Hapus soal ini?')">
                @csrf
                @method('DELETE')
            ...
            <x-button variant="secondary" href="{{ route('mapping-questions.manage', $mappable) }}" class="rounded-xl">
