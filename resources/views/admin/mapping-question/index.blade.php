...
@section('content')
    <div class="space-y-8 pb-20" x-data="{
        ...
        <div class="space-y-8">
            {{-- Keterangan Jumlah Soal per Mapel --}}
                ...
                <x-card :padding="false" title="Pilih Soal dari Bank">
                    <div
                            ...
                            <form action="{{ route('mapping-questions.manage', $mappable) }}" method="GET"
                                ...
                                <div class="flex items-end gap-2">
                                    ...
                                    <x-button type="button" variant="secondary" class="rounded-xl"
                                        href="{{ route('mapping-questions.manage', $mappable) }}">
                                        Reset
                                    </x-button>
                        ...
                        <div class="flex flex-col gap-3">
                            <form action="{{ route('mapping-questions.random', $mappable) }}" method="POST"
                                ...
                                <div>
                                    ...
                                    @endif
                                    <p class="text-[10px] text-gray-400 mt-1">Ambil soal random per mapel sesuai kebutuhan
                                    </p>
                                </div>
                            </form>
                            <div>
                    ...
                    <form action="{{ route('mapping-questions.store', $mappable) }}" method="POST">
                        @csrf
                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="border-b border-gray-50 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
                                    <th class="py-3 px-6 text-[11px] font-bold uppercase text-gray-400 tracking-wider">
                                        Mapel</th>
                                    <th class="py-3 px-6 text-[11px] font-bold uppercase text-gray-400 tracking-wider">Soal
                                    </th>
                                    <th
                                                ...
                                                <div
                                                    ...
                                                    @else
                                                        <div class="flex flex-col gap-2">
                                                            ...
                                                            <a href="{{ $q->question_image_url }}"
                                                                target="_blank" rel="noopener noreferrer"
                                                                class="text-xs text-sky-800 dark:text-gray-400 underline decoration-solid">
                                                                Lihat soal gambar
                                                            </a>
                                                        </div>
                                                    @endif
                                        ...
                                        <td class="py-3 px-6 align-top">
                                            <div class="flex items-center justify-end gap-2">
                                                <x-button variant="info" size="sm"
                                                    href="{{ route('mapping-questions.show', [$mappable, $map]) }}"
                                                    class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                                    <i class="ti ti-eye text-base"></i>
                                                </x-button>
                                                <x-button variant="danger" size="sm" type="button"
                                                    @click="confirmDelete('{{ route('mapping-questions.destroy', [$mappable, $map]) }}', '{{ \Illuminate\Support\Str::limit(strip_tags($q->question), 50) }}')"
                                                    class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="3" class="py-10">
                                            <p class="text-center text-sm text-gray-500 dark:text-gray-400">
                                                Belum ada soal yang di-mapping ke
                                                {{ $mappableType === 'exam' ? 'ujian' : 'kuis' }} ini.
                                            </p>
                                        </td>
                                    </tr>
                ...
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                        Anda akan menghapus soal <span class="font-bold text-gray-900 dark:text-white"
                            x-text="questionTitle"></span>. Tindakan ini tidak dapat dibatalkan.
                    </p>
                </div>
