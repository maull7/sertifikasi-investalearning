...
<div class="space-y-8 pb-20">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">
                Pilih ujian, lalu tentukan soal dari bank soal (manual atau acak) untuk dimasukkan ke ujian tersebut.
            </p>
        </div>
    ...
    {{-- Pilih Ujian --}}
    <x-card>
        <form action="{{ route('mapping-questions.create') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4 md:items-end">
            <div class="md:col-span-2">
                <x-select name="exam_id" label="Pilih Ujian" required>
                    <option value="">-- Pilih Ujian --</option>
                    @foreach($exams as $exam)
                        <option value="{{ $exam->id }}" {{ (int) $examId === $exam->id ? 'selected' : '' }}>
                            {{ $exam->title }} 
                            @if($exam->package)
                                - ({{ $exam->package->title }})
                            @endif
            ...
            <div>
                <x-button type="submit" variant="primary" class="w-full rounded-xl">
                    Pilih Ujian
                </x-button>
            </div>
        </form>
