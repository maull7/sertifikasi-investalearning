    ...
    <div class="space-y-8 pb-20" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Kelola mapping soal untuk ujian</p>
            </div>
            <div class="flex items-center gap-3">
                <x-button variant="primary" href="{{ route('mapping-questions.create') }}" icon="plus"
        ...
        <div class="flex flex-col md:flex-row gap-4">
            <form action="{{ route('mapping-questions.index') }}" method="GET" class="relative flex-1 group">
                ...
                @if (!empty($search ?? request('search')))
                    <a href="{{ route('mapping-questions.index') }}"
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-gray-50 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/30">
                                <th class="py-4 px-8 text-[11px] font-bold uppercase text-gray-400 tracking-wider">Ujian /
                                    Exam</th>
                                <th class="py-4 px-8 text-[11px] font-bold uppercase text-gray-400 tracking-wider">Paket
                                </th>
                                <th class="py-4 px-8 text-[11px] font-bold uppercase text-gray-400 tracking-wider">Total
                                    Soal</th>
                                <th
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                            @forelse ($examsWithMapping as $value)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors group">
                                    ...
                                    <td class="py-4 px-8">
                                        <span
                                            class="text-sm text-gray-700 dark:text-gray-300">{{ $value->package->title ?? '-' }}</span>
                                    </td>
                                    <td class="py-4 px-8">
                                        @php($mappedCount = $value->mappingQuestions->count())
                                        <span
                                            class="inline-flex items-center justify-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-50 text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                                            {{ $mappedCount }} Soal
                                        </span>
                                    </td>
                                    <td class="py-4 px-8">
                                        <div class="flex items-center justify-end gap-2">
                                            <x-button variant="secondary" size="sm"
                                                href="{{ route('mapping-questions.manage', $value) }}"
                                ...
                                <tr>
                                    <td colspan="4" class="py-16 text-center">
                                        <p class="text-sm text-gray-500 dark:text-gray-400">
                                            ...
                                            @else
                                                Belum ada ujian dengan mapping soal.
                                            @endif
                                        </p>
                                    </td>
                                </tr>
            ...
            <div class="bg-white dark:bg-gray-900 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800 max-h-[60vh] flex flex-col"
                ...
                <div class="flex items-center justify-between p-4 border-b border-gray-100 dark:border-gray-800">
                    <div class="flex items-center gap-2">
                        <div
                            class="w-8 h-8 bg-indigo-50 dark:bg-indigo-500/10 rounded-lg flex items-center justify-center">
                ...
                <div class="p-4 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                    <x-button variant="secondary" @click="previewModalOpen = false"
                        class="w-full rounded-lg text-sm py-2">
                        Tutup
                    </x-button>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800"
                ...
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                        Anda akan menghapus material <span class="font-bold text-gray-900 dark:text-white"
                            x-text="materialName"></span>. File terkait juga akan dihapus. Tindakan ini tidak dapat
                        dibatalkan.
                    </p>
                </div>
