...
@section('content')
    <div class="space-y-8 pb-20">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Tambah Data Buku</h1>
        ...
        <x-card>
            <form action="{{ route('books.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-input label="NAMA BUKU" name="title" placeholder="Masukkan nama buku" value="{{ old('title') }}"
                        required />
                    @error('title')
                        <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                    @enderror
                    <x-textarea label="Penulis" name="author" placeholder="Masukan nama penulis contoh : J.K Rowling"
                        value="{{ old('author') }}" rows="3" required>{{ old('author') }}</x-textarea>
                    @error('author')
                        <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
                <x-textarea label="Deksripsi buku" name="description" placeholder="Masukan deskripsi buku"
                    value="{{ old('description') }}" rows="3" required>{{ old('description') }}</x-textarea>
                @error('description')
                    <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                @enderror
