...
<div class="max-w-4xl mx-auto space-y-8 pb-20">
    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Edit Mata Pelajaran</h1>
    ...
    {{-- Form Card --}}
    <x-card title="Edit Mata Pelajaran">
        <form action="{{ route('subjects.update', $data->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="md:col-span-2">
                    ...
                    >
                        ...
                        @foreach($types as $type)
                            <option value="{{ $type->id }}" {{ old('master_type_id', $data->master_type_id) == $type->id ? 'selected' : '' }}>
                                {{ $type->name_type }}
                            </option>
                        @endforeach
                    </x-select>
