...
@section('content')
    <div class="max-w-4xl mx-auto space-y-8 pb-20">
        ...
        <x-card title="Material Baru">
            <form action="{{ route('master-materials.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <x-select label="Mata Pelajaran" name="id_subject" required>
                            ...
                            @foreach ($subjects as $subject)
                                <option value="{{ $subject->id }}"
                                    {{ old('id_subject') == $subject->id ? 'selected' : '' }}>
                                    {{ $subject->name }}
                                </option>
                            @endforeach
                        </x-select>
                    ...
                    {{-- Topic Input --}}
                    <div class="md:col-span-2">
                        <x-textarea label="Topik Materi" name="topic" placeholder="Topik singkat mengenai materi ini..."
                            rows="3" required>{{ old('topic') }}</x-textarea>
                    </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label
                                ...
                                <input type="radio" name="materi_type" value="File" class="accent-indigo-600"
                                    {{ old('materi_type', 'File') === 'File' ? 'checked' : '' }}>
