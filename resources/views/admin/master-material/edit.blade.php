...
@section('content')
    <div class="max-w-4xl mx-auto space-y-8 pb-20">
            ...
            <div class="space-y-1">
                ...
                <nav class="flex items-center gap-2 text-xs font-medium text-gray-400">
                    <a href="{{ route('master-materials.index') }}"
                        class="hover:text-indigo-600 transition-colors">Material</a>
        ...
        <x-card title="Edit Material">
            <form action="{{ route('master-materials.update', $data->id) }}" method="POST" enctype="multipart/form-data"
                class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <x-select label="Mata Pelajaran" name="id_subject" required>
                            <option value="">Pilih Mata Pelajaran</option>
                            @foreach ($subjects as $subject)
                                <option value="{{ $subject->id }}"
                                    {{ old('id_subject', $data->id_subject) == $subject->id ? 'selected' : '' }}>
                                    {{ $subject->name }}
                                </option>
                            @endforeach
                        </x-select>
                    ...
                    {{-- Title Input --}}
                    <div class="md:col-span-2">
                        <x-input label="Judul Material" name="title" placeholder="Contoh: Modul 1 - Pengenalan"
                            value="{{ old('title', $data->title) }}" required />
                    </div>

                    {{-- Topic Input --}}
                    <div class="md:col-span-2">
                        <x-textarea label="Topik Material" name="topic" placeholder="Topik singkat mengenai materi ini..."
                            rows="3" required>{{ old('topic', $data->topic) }}</x-textarea>
                    </div>
                    {{-- Description Input --}}
                    <div class="md:col-span-2">
                        <x-textarea label="Deskripsi Material" name="description"
                            placeholder="Deskripsi singkat mengenai material ini..." rows="3"
                            required>{{ old('description', $data->description) }}</x-textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label
                                class="flex items-center gap-3 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 cursor-pointer">
                                <input type="radio" name="materi_type" value="File" class="accent-indigo-600"
                                    {{ old('materi_type', $data->materi_type ?? 'File') === 'File' ? 'checked' : '' }}>
                            ...
                            <label
                                class="flex items-center gap-3 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 cursor-pointer">
                                <input type="radio" name="materi_type" value="Video" class="accent-indigo-600"
                                    {{ old('materi_type', $data->materi_type ?? 'Video') === 'Video' ? 'checked' : '' }}>
                                <div class="flex items-center gap-2">
                                    <div
                    ...
                    @if ($data->value)
                        <div class="md:col-span-2" id="materi-file">
                            ...
                            <div
                                class="flex items-center justify-between gap-3 p-4 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-2xl">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-10 h-10 bg-gray-100 dark:bg-gray-800 rounded-lg flex items-center justify-center">
                                        <i
                                            class="{{ $data->file_icon }} text-xl {{ $data->file_type === 'pdf' ? 'text-rose-600' : 'text-blue-600' }}"></i>
                                    </div>
                                    <div class="min-w-0">
                                        <p
                                            class="text-sm font-semibold text-gray-900 dark:text-white truncate max-w-[240px]">
                                            {{ $data->file_name }}</p>
                                        <p class="text-[11px] text-gray-500 dark:text-gray-400">
                                            {{ strtoupper($data->file_type) }} • {{ $data->file_size_formatted }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('master-materials.preview', $data->id) }}" target="_blank" rel="noopener noreferrer"
                                    ...
                                    <a href="{{ route('master-materials.download', $data->id) }}"
                                        class="inline-flex items-center gap-1 px-3 py-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-800/40 dark:hover:bg-gray-800/70 text-gray-700 dark:text-gray-200 text-xs font-semibold rounded-lg transition-colors">
                        ...
                        <div class="md:col-span-2" id="materi-video">
                            ...
                            <input type="url" name="url_link" value="{{ old('url_link', $data->value) }}"
