...
<div class="max-w-4xl mx-auto space-y-8 pb-20">
    ...
    <div class="flex items-center justify-between">
        <div class="space-y-1">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Tambah Jenis</h1>
            <nav class="flex items-center gap-2 text-xs font-medium text-gray-400">
                <a href="{{ route('master-types.index') }}" class="hover:text-indigo-600 transition-colors">Jenis</a>
                <i class="ti ti-chevron-right"></i>
                <span class="text-gray-500 dark:text-gray-500">Edit Data Jenis</span>
            </nav>
        </div>
        <x-button variant="secondary" href="{{ route('master-types.index') }}" class="rounded-xl">
            <i class="ti ti-arrow-left mr-2"></i> Kembali
        </x-button>
    </div>

    {{-- Form Card --}}
    <x-card title="Jenis Baru">
        <form action="{{ route('master-types.update', $data->id) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    <x-input 
                        ...
                        value="{{ old('name_type', $data->name_type) }}"
                        required 
                    />
                </div>
