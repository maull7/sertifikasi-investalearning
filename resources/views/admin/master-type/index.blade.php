...
<div class="space-y-8 pb-20" x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Jenis</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Kelola Data Jenis Anda</p>
        </div>
        <div class="flex items-center gap-3">
            <x-button variant="secondary" href="{{ route('master-types.export-template') }}" icon="download" class="rounded-xl shadow-lg shadow-indigo-500/20">
                Download Template
            </x-button>
             <x-button variant="success" type="button" @click="ModalJenis = true" class="rounded-xl">
                <i class="ti ti-file-upload mr-2"></i> Import Excel
            </x-button>
            <x-button variant="primary" href="{{ route('master-types.create') }}"
            icon="plus" class="rounded-xl shadow-lg shadow-indigo-500/20">
                Tambah Jenis Baru
            </x-button>
    ...
    <div class="flex flex-col md:flex-row gap-4">
        <form action="{{ route('master-types.index') }}" method="GET" class="relative flex-1 group">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors group">
                            ...
                            <td class="py-4 px-8">
                                <div class="flex items-center justify-end gap-2">
                                    <x-button variant="secondary" size="sm" href="{{ route('master-types.edit', $value->id) }}" class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                        <i class="ti ti-pencil text-base"></i>
                                    </x-button>

                                    <x-button variant="danger" size="sm" type="button"
                                        @click="confirmDelete('{{ route('master-types.destroy', $value->id) }}', '{{ $value->name_type }}')"
    ...
    <div x-show="ModalJenis" 
        ...
        <div class="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800"
            @click.away="ModalJenis = false">
            
            <form action="{{ route('master-types.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="p-8">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2 text-center">Import Jenis dari Excel</h3>
                    <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed text-center mb-6">
                        Upload file Excel (.xlsx atau .xls) untuk import jenis dalam jumlah banyak.
                    </p>
                ...
                <div class="flex gap-3 p-6 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-800">
                    <x-button variant="secondary" type="button" class="flex-1 rounded-xl" @click="ModalJenis = false">
                        Batal
                    </x-button>
