...
@section('content')
    ...
    <div class="space-y-8 pb-20" x-data="{
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Bank Question</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Kelola Data Soal Anda</p>
            </div>
            <div class="flex items-center gap-3">
                <x-button variant="secondary" href="{{ route('bank-questions.download-template') }}" class="rounded-xl">
                    <i class="ti ti-download mr-2"></i> Download Template
                </x-button>
                <x-button variant="success" type="button" @click="importModalOpen = true" class="rounded-xl">
                    <i class="ti ti-file-upload mr-2"></i> Import Excel
                </x-button>
                <x-button variant="primary" href="{{ route('bank-questions.create') }}" icon="plus"
        ...
        <div class="flex flex-col lg:flex-row gap-4">
            <form action="{{ route('bank-questions.index') }}" method="GET"
                ...
                <div class="w-full md:w-64">
                    <x-select name="subject_id" label="Filter Mata Pelajaran Soal" inline class="h-12">
                        <option value="">Semua Mata Pelajaran</option>
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}"
                                {{ (int) ($subjectId ?? 0) === $subject->id ? 'selected' : '' }}>
                                {{ $subject->name }}
                            </option>
                        @endforeach
                    </x-select>
        ...
        {{-- Main Data Card --}}
        <x-card :padding="false" title="Daftar Soal">
                        ...
                        @forelse ($data as $value)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors group">
                                    ...
                                    <div class="max-w-md">
                                        ...
                                        <div class="mt-2 flex flex-wrap items-center gap-2">
                                            @if (($value->question_type ?? 'Text') === 'Image')
                                                <span
                                                    class="inline-flex items-center gap-1 text-xs font-semibold bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300 px-2 py-1 rounded-full">
                                                    <i class="ti ti-photo"></i> Image
                                                </span>
                                            @else
                                                <span
                                                    class="inline-flex items-center gap-1 text-xs font-semibold bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300 px-2 py-1 rounded-full">
                                ...
                                <td class="py-4 px-8 text-center">
                                    <span
                                        class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 font-bold text-sm">
                                        {{ strtoupper($value->answer) }}
                                    </span>
                                </td>
                                <td class="py-4 px-8">
                                    <div class="flex items-center justify-end gap-2">
                                        <x-button variant="secondary" size="sm"
                                            href="{{ route('bank-questions.edit', $value->id) }}"
                                            class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                            <i class="ti ti-pencil text-base"></i>
                                        </x-button>

                                        <x-button variant="danger" size="sm" type="button"
                                            @click="confirmDelete('{{ route('bank-questions.destroy', $value->id) }}', '{{ Str::limit(strip_tags($value->question), 50) }}')"
                                            class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                            <i class="ti ti-trash text-base"></i>
                                        </x-button>
                                        <x-button variant="primary" size="sm" type="button"
                                            href="{{ route('bank-questions.show', $value->id) }}"
                                            class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
            ...
            <div class="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800"
                ...
                <form action="{{ route('bank-questions.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-8">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2 text-center">Import Soal dari Excel
                        </h3>
                ...
                <div class="p-8 text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                        Anda akan menghapus soal <span class="font-bold text-gray-900 dark:text-white"
                            x-text="questionTitle"></span>. Gambar terkait juga akan dihapus. Tindakan ini tidak dapat
                        dibatalkan.
                    </p>
                </div>
