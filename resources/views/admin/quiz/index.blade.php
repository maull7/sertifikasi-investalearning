...
<div class="space-y-8 pb-20" x-data="{
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Quiz</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400 font-medium">Kelola  Quiz Anda</p>
        </div>
        <div class="flex items-center gap-3">
            <x-button variant="primary" href="{{ route('quizzes.create') }}" icon="plus" class="rounded-xl shadow-lg shadow-indigo-500/20">
                Tambah Kuis Baru
            </x-button>
        </div>
    </div>
    ...
    <div class="flex flex-col md:flex-row gap-4">
        <form action="{{ route('quizzes.index') }}" method="GET" class="relative flex-1 group">
            ...
            >
            @if(request('search'))
                <a href="{{ route('quizzes.index') }}" class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-rose-500 transition-colors">
                    <i class="ti ti-x"></i>
                </a>
            @endif
        </form>
    ...
    {{-- Table --}}
    <x-card :padding="false" title="Daftar Kuis Tersedia">
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                    @forelse ($data as $value)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors group">
                            ...
                            <td class="py-4 px-8">
                                <span class="inline-flex items-center gap-1 text-sm text-gray-700 dark:text-gray-300 font-medium">
                                    <i class="ti ti-clock text-amber-500"></i>
                                    {{ $value->duration }} menit
                                </span>
                            </td>
                            <td class="py-4 px-8">
                                <span class="inline-flex items-center justify-center px-3 py-1 rounded-lg bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 font-bold text-sm">
                                    {{ $value->passing_grade }}
                                </span>
                            </td>
                            ...
                            <td class="py-4 px-8">
                                <div class="flex items-center justify-end gap-2">
                                    <x-button variant="info" size="sm" href="{{ route('quizzes.show', $value->id) }}" class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                        <i class="ti ti-eye text-base"></i>
                                    </x-button>
                                    <x-button variant="secondary" size="sm" href="{{ route('quizzes.edit', $value->id) }}" class="rounded-lg h-9 w-9 p-0 flex items-center justify-center">
                                        <i class="ti ti-pencil text-base"></i>
                                    </x-button>
                                    <x-button variant="danger" size="sm" type="button"
                                        @click="confirmDelete('{{ route('quizzes.destroy', $value->id) }}', '{{ $value->title }}')"
                                ...
                                <div class="flex flex-col items-center justify-center text-center max-w-[280px] mx-auto">
                                    <div class="space-y-1">
                                        ...
                                        <p class="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
                                            ...
                                            @else
                                                Belum ada Kuis. Mulai tambahkan Kuis baru.
                                            @endif
                                        </p>
                                    </div>
                                    <div class="mt-6">
                                        @if(request('search'))
                                            <x-button variant="secondary" href="{{ route('quizzes.index') }}">
                                                Reset Pencarian
                                            </x-button>
                                        @else
                                            <x-button variant="secondary" href="{{ route('quizzes.create') }}">
    ...
    <div x-show="deleteModalOpen"
        ...
        <div class="bg-white dark:bg-gray-900 w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800"
            ...
            <div class="p-8 text-center">
                ...
                <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Hapus Kuis?</h3>
                <p class="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                    Anda akan menghapus Kuis <span class="font-bold text-gray-900 dark:text-white" x-text="examTitle"></span>. Tindakan ini tidak dapat dibatalkan.
                </p>
            </div>
