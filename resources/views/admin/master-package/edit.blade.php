        ...
        <x-card title="Paket Baru">
            <form action="{{ route('master-packages.update', $data->id) }}" method="POST" class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <x-select label="Jenis" name="id_master_types" required>
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type->id }}"
                                    {{ old('id_master_types', $data->id_master_types) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name_type }}
                                </option>
                            @endforeach
                        </x-select>
                    ...
                    {{-- Name Input --}}
                    <div class="md:col-span-2">
                        <x-input label="Nama Paket" name="title" placeholder="Contoh: PAKET A"
                    ...
                    <div class="md:col-span-2">
                        <x-input label="Biaya Investasi (Rp)" name="price" type="number"
                            placeholder="Contoh: 500000 (kosongkan jika call)" value="{{ old('price', $data->price) }}"
                            min="0" />
                    </div>

                    <div class="md:col-span-2">
                        <x-select label="Status Paket" name="status" required>
